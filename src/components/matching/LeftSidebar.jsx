...
export default function LeftSidebar({
  ...
  matches = [],
  loading,
  onRefetchRooms, // ✅ callback เพื่อส่ง refetchRooms ขึ้นไปให้ parent
}) {
  const router = useRouter();
  const { rooms, loading: roomsLoading, markRoomAsRead, refetchRooms } = useRealtimeChatList();

  // ✅ expose refetchRooms ให้ parent เรียกได้ผ่าน prop
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <img src="/merry_icon/icon-search-love.svg" alt="" className="w-10 h-10" />
        </motion.div>
        <h4 className="text-body2 font-bold text-red-500">Discover New Match</h4>
        <span className="text-body5">
          Start find and Merry to get know and connect with new friend!
        </span>
      </motion.div>

      <motion.hr variants={sectionVariants} className="shrink-0 border-gray-100" />

      {/* ── Merry Match — scroll แนวนอน ── */}
      <motion.div variants={sectionVariants} className="shrink-0 flex flex-col gap-4">
          ...
          <div className="flex gap-4 w-max px-2">
            {loading ? (
              Array(4).fill(0).map((_, i) => (
                <Skeleton key={i} className="w-16 h-16 rounded-2xl bg-gray-200 shrink-0" />
              ))
            ) : (
                ...
                {matches.map((match, index) => (
                  ...
                  >
                    ...
                    <motion.img
                      ...
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      transition={{ delay: index * 0.05 + 0.2, type: "spring", stiffness: 600 }}
                    />
                    {index < 3 && (
                      <motion.div
      ...
      {/* ── Chat Rooms — scroll แนวตั้ง ── */}
      <motion.div variants={sectionVariants} className="flex flex-col gap-4 flex-1 min-h-0">
        ...
        <div className="overflow-y-auto flex-1 flex flex-col gap-2 -mx-2 px-2 pb-4 [&::-webkit-scrollbar]:hidden">
          {roomsLoading ? (
            ...
            <p className="text-body5 text-gray-400 text-center py-4">No conversations yet</p>
          ) : (
            <AnimatePresence mode="popLayout">
              {rooms.map((room, index) => (
                <motion.button
                  ...
                  // ✅ กดแล้วไปหน้าแชทของ room นั้น
                  onClick={() => { markRoomAsRead(room.roomId); router.push(`/chat/${room.roomId}`); }}
                ...
                >
                  ...
                  <div className="relative z-10 min-w-0 flex-1">
                    ...
                    <p className={`text-[12px] truncate mt-0.5 ${room.unreadCount > 0 ? "text-gray-700 font-semibold" : "text-gray-400"}`}>
