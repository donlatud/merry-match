            ...
            >
              ...
              {[...Array(6)].map((_, i) => (
                <motion.div key={i} className="absolute text-xl select-none pointer-events-none"
                  ...
                  animate={{ y: -(100 + i * 25), opacity: [0, 1, 0], scale: [0, 1.2, 0.6] }}
                  transition={{ delay: 0.4 + i * 0.1, duration: 1.4, ease: "easeOut" }}>
                  ❤️
                </motion.div>
              ))}
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-end pb-12 px-8 gap-3 w-full">
                <motion.img src="/merry_icon/icon-match.svg" alt="" className="w-16 h-16 drop-shadow-lg"
                  initial={{ scale: 0, rotate: -20 }} animate={{ scale: 1, rotate: 0 }}
                  transition={{ delay: 0.25, type: "spring", stiffness: 380, damping: 14 }} />

                <motion.h2 className="text-4xl font-sans text-white tracking-wide text-center"
                ...
                {matchedProfile?.name && (
                  <motion.p className="text-white/80 text-base font-medium -mt-1 text-center"
                ...
                <motion.button onClick={handleStartConversation} disabled={navigating}
                ...
                <motion.div className="absolute bottom-0 left-0 h-1 bg-white/40 rounded-full"
